...
bool TiParser::parse(){
	unsigned char c = 0;
	this->lex.buffer.read(c);
	if ( c == '#' ){
		this->lex.buffer.accept();
		char ass[] = {'T','1','V'};
		...
		if ( is_asm ){
			while ( this->lex.buffer.next(c) ){
				this->output->text += c;
			}
			return true;
		} else {
			// colocar em uma rotina ( excluir linha )
			while ( this->lex.buffer.next(c) ){
				if ( c == '\n' )
					break;
			}
